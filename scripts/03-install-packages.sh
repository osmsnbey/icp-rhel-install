...
for ((i=1; i < $NUM_MASTER; i++)); do
  ...
  # Fall back to pinned version (no fallback for ppc)
  if [ "$?" == "1" ]; then
    ssh ${SSH_USER}@${MASTER_HOSTNAMES[i]} yum install --setopt=obsoletes=0 -y docker-ce-17.12.1.ce-1.el7.centos.x86_64 docker-ce-selinux-17.12.1.ce-1.el7.centos.noarch
  fi
...
for ((i=0; i < $NUM_WORKERS; i++)); do
  ...
  # Fall back to pinned version (no fallback for ppc)
  if [ "$?" == "1" ]; then
    ssh ${SSH_USER}@${WORKER_HOSTNAMES[i]} yum install --setopt=obsoletes=0 -y docker-ce-17.12.1.ce-1.el7.centos.x86_64 docker-ce-selinux-17.12.1.ce-1.el7.centos.noarch
  fi
